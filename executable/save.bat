...
set TESTMP_CLIENTS=.
for /r  "%TESTMP_HOME%\lib" %%i in (*.jar) do set TESTMP_CLIENTS=!TESTMP_CLIENTS!;%%i

cd "%TESTMP_HOME%\bin"
if not "%JAVA_HOME%" == "" goto hasJavaHome
java -cp "%TESTMP_CLIENTS%" SaveLoad save %CMD_LINE_ARGS%
goto end
:hasJavaHome
"%JAVA_HOME%\bin\java" -cp "%TESTMP_CLIENTS%" SaveLoad save %CMD_LINE_ARGS%

...
if exist "%TESTMP_HOME%\bin\UserAdmin.class" goto validHome
cd ..
set "TESTMP_HOME=%cd%"
cd "%CURRENT_DIR%"
:hasHome
...
if exist "%TESTMP_HOME%\bin\UserAdmin.class" goto canRun
echo Cannot find UserAdmin.class
...
java -cp "%TESTMP_LIB%" UserAdmin %CMD_LINE_ARGS%
goto end
:hasJavaHome
"%JAVA_HOME%\bin\java" -cp "%TESTMP_LIB%" UserAdmin %CMD_LINE_ARGS%
